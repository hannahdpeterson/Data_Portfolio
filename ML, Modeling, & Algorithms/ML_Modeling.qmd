---
title: "Modeling 5 Data Mining Algorithms Utilizing 3 ML Methods of Interest"
...
Given the review of our variables and their values, we should expect (was gonna explain what classes i expect them to be and also suggest changing the G1-G3 to numeric values so we can use them un-leveled)
...
In order to differentiate our model from theirs while still replicating part of the study, we wish to take the approach that the model is used prior to a students choice to enroll in a class. Essentially, our model see's to predict a students outcome in the class using variables that are known prior to class enrollment so that a user could predict their grade before the third trimester. Essentially, this means removing G1 and G2 as predictive variables.
...
# for one-hot-encoding, converst to 1-of-c using all nominals
